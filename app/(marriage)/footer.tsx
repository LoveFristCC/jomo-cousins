import Link from "next/link";
import Image from "next/image";

export default function Footer() {
  return (
    <footer className="relative bg-[#f5f5f0] py-16 text-[#303030]">
      <div className="container mx-auto px-5">
        <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-4">
          {/* Logo & Mission */ }
          <div>
            <div className="mb-6">
              <Image
                src="/images/marriage/main-page/Jomo-and-Charmaine-logo.webp"
                alt="Jomo & Charmaine"
                width={ 250 }
                height={ 100 }
                className="h-auto w-48"
                quality={ 100 }
              />
            </div>
            <div className="mb-4 h-[2px] w-16 bg-[#ea8125]"></div>
            <p className="text-sm leading-relaxed text-gray-600">
              We strive to help you find your rhythm as couple and develop healthy habits that will allow you to be the best version of yourselves so you can thrive as a unit.
            </p>
          </div>

          {/* Quick Links */ }
          <div>
            <h3 className="mb-2 text-xl font-bold">Quick Links</h3>
            <div className="mb-6 h-[2px] w-16 bg-[#ea8125]"></div>
            <ul className="space-y-3 text-sm">
              <li>
                <Link
                  href="/marriage/about"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  About Us
                </Link>
              </li>
              <li>
                <Link
                  href="/marriage/blog"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  Couples' Corner
                </Link>
              </li>
              <li>
                <Link
                  href="/products"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  Books & Resources
                </Link>
              </li>
              <li>
                <Link
                  href="/marriage/contact"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  Contact Us
                </Link>
              </li>
            </ul>
          </div>

          {/* Services */ }
          <div>
            <h3 className="mb-2 text-xl font-bold">Services</h3>
            <div className="mb-6 h-[2px] w-16 bg-[#ea8125]"></div>
            <ul className="space-y-3 text-sm">
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Communication Issues</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Premarital Counseling</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Sexual Issues</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Infidelity & Unfaithfulness</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Trust Issues</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#ea8125] flex-shrink-0"></span>
                <span className="text-gray-700">Blended Family Support</span>
              </li>
            </ul>
          </div>

          {/* Connect & Resources */ }
          <div>
            <h3 className="mb-2 text-xl font-bold">Connect & Resources</h3>
            <div className="mb-6 h-[2px] w-16 bg-[#ea8125]"></div>

            {/* Other Sites */ }
            <ul className="space-y-3 text-sm mb-6">
              <li>
                <Link
                  href="/"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  Main Site
                </Link>
              </li>
              <li>
                <Link
                  href="/prayer"
                  className="group inline-flex items-center text-gray-700 transition-colors hover:text-[#ea8125]"
                >
                  <span className="mr-2 opacity-0 transition-opacity group-hover:opacity-100">→</span>
                  Prayer Ministry
                </Link>
              </li>
            </ul>

            {/* CTA Button */ }
            <Link
              href="/marriage/contact"
              className="mb-6 block rounded-lg bg-[#ea8125] px-6 py-3 text-center text-sm font-bold uppercase tracking-wider text-white shadow-lg transition-all hover:bg-[#d67320] hover:shadow-xl hover:scale-105"
            >
              Request an Appointment
            </Link>

            {/* Social Media */ }
            <div className="flex justify-center gap-4">
              <a
                href="https://www.youtube.com/jomocharmaine"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="YouTube"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </a>
              <a
                href="https://www.facebook.com/JomoAndCharmaine"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="Facebook"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href="https://www.instagram.com/jomoandcharmaine_"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="Instagram"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
              <a
                href="https://twitter.com/JomoCharmaine"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="Twitter"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href="https://www.linkedin.com/in/dr-jomo-cousins-277279138/"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="LinkedIn"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
              </a>
              <a
                href="/marriage/feed.xml"
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-10 w-10 items-center justify-center rounded-full bg-[#ea8125]/10 transition-all hover:bg-[#ea8125] hover:scale-110"
                aria-label="RSS feed"
              >
                <svg className="h-5 w-5 text-[#ea8125] transition-colors group-hover:text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M6.503 20.752c0 1.794-1.456 3.248-3.251 3.248-1.796 0-3.252-1.454-3.252-3.248 0-1.794 1.456-3.248 3.252-3.248 1.795.001 3.251 1.454 3.251 3.248zm-6.503-12.572v4.811c6.05.062 10.96 4.966 11.022 11.009h4.817c-.062-8.71-7.118-15.758-15.839-15.82zm0-3.368c10.58.046 19.152 8.594 19.183 19.188h4.817c-.03-13.231-10.755-23.954-24-24v4.812z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        <div className="mt-8 border-t border-gray-300 pt-8 text-center text-sm text-gray-600">
          <div className="mb-4 flex flex-wrap items-center justify-center gap-4">
            <Link href="/privacy" className="transition-colors hover:text-[#ea8125]">
              Privacy Policy
            </Link>
            <span>•</span>
            <Link href="/terms" className="transition-colors hover:text-[#ea8125]">
              Terms of Service
            </Link>
          </div>
          <p>&copy; { new Date().getFullYear() } Jomo & Charmaine. All rights reserved.</p>
        </div>
      </div>

    </footer>
  );
}
