...
// Query: *[_type == "product" && status == "active" && slug.current != $slug] {    _id,    name,    "slug": slug.current,    category,    images,    basePrice,    excerpt,    author,    featured  } | order(select(category == $category => 0, 1) asc, featured desc, name asc) [0...4]
...
// Query: *[_type == "prayerVideo" && _id != $excludeId && count((prayerCategories[]->slug.current)[@ in $categoryIds]) > 0] | order(publishedAt desc) [0...$limit] {      _id,  _updatedAt,  title,  "slug": slug.current,  youtubeUrl,  youtubeVideoId,  excerpt,  "categories": prayerCategories[]->{ title, "slug": slug.current },  tags,  featuredImage,  publishedAt,  isPrayerOfTheDay,  duration,  personalNote,  viewCount  }
